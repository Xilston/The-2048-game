...
class Game {
    ...
    startGame() {
        ...
        for(let i = 0; i < this.size; i++){
            const row = [];
            for(let j = 0; j < this.size; j++)row.push(new Chip(0));
            this.chipsArray.push(row);
        }
        for(let i = 0; i < 2; i++)this.addChip(new Chip(gen2or4()));
    }
    addChip(chip) {
    ...
    updateTiles() {
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++)this.chipsArray[i][j].setStatus("old");
    }
    moveLeft() {
        const chips = this.chipsArray;
        let isMove = false;
        this.updateTiles();
        for(let i = 0; i < chips.length; i++)for(let j = 1; j < chips.length; j++){
    ...
    moveUp() {
        ...
        let isMove = false;
        this.updateTiles();
        for(let i = 1; i < chips.length; i++)for(let j = 0; j < chips.length; j++){
            const current = chips[i][j];
            if (current.value !== 0) {
                let targetCellIndex = i - calcTargetCellIndex({
                    y: i,
                    x: j
                }, chips, "up");
    ...
    moveDown() {
        ...
        let isMove = false;
        this.updateTiles();
        for(let i = this.size - 2; i >= 0; i--)for(let j = 0; j < chips.length; j++){
            const current = chips[i][j];
            if (current.value !== 0) {
                let targetCellIndex = i + calcTargetCellIndex({
                    y: i,
    ...
    moveRight() {
        ...
        let isMove = false;
        this.updateTiles();
        for(let i = 0; i < this.size; i++)for(let j = this.size - 2; j >= 0; j--){
            const current = chips[i][j];
            if (current.value !== 0) {
    ...
    isGameOver() {
        ...
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++){
            ...
            over = closestTiles.every((tile)=>tile.value !== currentTile.value);
            if (over === false) {
                this.gameOver = false;
                return over;
            }
    ...
    resetTiles() {
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++)this.chipsArray[i][j].resetMerge();
    }
...
function syncWithTable() {
    ...
    console.log(chips);
    for(let i = 0; i < chips.length; i++)for(let j = 0; j < chips[i].length; j++){
        const cell = tableRows[i].children[j];
        const chip = chips[i][j];
        const prevChipValue = cell.innerText;
        const merge = chip.status !== "old" ? "merged " : "";
        if (chip.value !== 0) {
...
function clearTable() {
    for(let i = 0; i < tableRows.length; i++)for(let j = 0; j < tableRows.length; j++){
        const cell = tableRows[i].children[j];
        cell.innerHTML = "";
        cell.className = "field-cell";
    }
...
/* function calcFreeCount(currenIndex, targetArray) {
  ...
  return result;
    ...
    let res = 0;
    if (direction === "left") {
        for(let i = currIndex - 1; i >= 0; i--)if (array[i].value === 0) res++;
        else {
    ...
    if (direction === "up") {
        const { y, x } = currIndex;
        for(let i = y - 1; i >= 0; i--)if (array[i][x].value === 0) res++;
        else {
            if (array[i][x].value === array[y][x].value && array[i][x].ableToUnion) res++;
            else return res;
        }
    }
    if (direction === "down") {
        const { y, x } = currIndex;
        for(let i = y + 1; i < array.length; i++)if (array[i][x].value === 0) res++;
        else {
            if (array[i][x].value === array[y][x].value && array[i][x].ableToUnion) res++;
            else return res;
        }
    ...
    if (direction === "right") {
        for(let i = currIndex + 1; i < array.length; i++)if (array[i].value === 0) res++;
        else {
            if (array[i].value === array[currIndex].value && array[i].ableToUnion) res++;
            else return res;
        }
